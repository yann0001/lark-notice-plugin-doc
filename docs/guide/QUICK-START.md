...
打开 `Manage Jenkins` 页面后，通过 `Lark Notice` 管理入口进入插件配置页。

![](./img/faq-manage.png)

进入 `Lark Notice` 页面后，可配置默认通知时机、高级设置，并新增或维护可复用的机器人。
...
保存后建议先点击机器人卡片中的 `发送测试`，确认当前 Jenkins 网络、签名策略和代理设置均可正常访问目标平台。
